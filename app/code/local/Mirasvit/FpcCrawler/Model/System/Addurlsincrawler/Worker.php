...
class Mirasvit_FpcCrawler_Model_System_Addurlsincrawler_Worker extends Varien_Object
{
    ...
    public function run($all = false, $crawl = false)
    {
        if ($all) {
            $this->maxPerStep = 1000000;
        }
        set_time_limit(0);
    ...
    {
        $suffix = Mage::helper('catalog/category')->getCategoryUrlSuffix($storeId);
        $baseUrl = Mage::app()->getStore($storeId)->getBaseUrl();
        $categoryPath = $item->getUrlPath();

        $url = rtrim($baseUrl, '/') . '/' . ltrim($categoryPath, '/') . $suffix;
