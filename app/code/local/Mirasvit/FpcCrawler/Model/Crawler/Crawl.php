...
class Mirasvit_FpcCrawler_Model_Crawler_Crawl extends Varien_Object
{
    ...
    {
        if (!$storeId) {
            $storeId = Mage::getSingleton('fpccrawler/config')->getStoresIds();
        }
        if ($isIgnoreTotalCount) {
            $this->lockFileName = 'fpccrawler_fpc_generate'.(int) $storeId.'.lock';
            echo 'Not logged in users crawl start:'.PHP_EOL;
        }
        $this->lock();
        ...
        if ($threads == 1) {
            while ($row = $collection->fetch()) {
                ...
                if (!$urlModel->isCacheExist()) {
                    ...
                    if ($totalCount >= $config->getCrawlerMaxUrlsPerRun() && !$isIgnoreTotalCount) {
                        break;
                    }
                }
            }
    ...
    protected function _getLockFile()
    {
        if ($this->_lockFile === null) {
            ...
            $lockFile = ($this->lockFileName) ? $this->lockFileName : 'fpccrawler.lock';
