...
class Mirasvit_FpcCrawler_Model_Config
{
    ...
    public function isEnabled($logged = null, $storeId = null)
    {
        ...
        return ($this->getStoresIds($logged)) ? true : false;
    }

    public function getStoresIds($logged = null)
    {
        $path = $logged ? 'fpccrawler/crawler_logged/enabled' : 'fpccrawler/crawler/enabled';
        $storesIds = false;
        foreach (Mage::app()->getStores() as $store) {
            $storeId = $store->getId();
            if ($store->getIsActive() && Mage::getStoreConfig($path, $storeId)) {
                $storesIds[$storeId] = $storeId;
            }
        }

         //if crawler disabled $storesIds will be empty for isEnabled function
        return ($storesIds) ? implode(',', $storesIds) : false;
    }
