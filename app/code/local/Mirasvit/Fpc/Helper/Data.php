...
class Mirasvit_Fpc_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getNormalizedUrl($protocol = false)
    {
        ...
        if ($uri) {
            ...
            if (($query && isset($requestUri) && strpos($_SERVER['REQUEST_URI'], '?') !== false)
                || ($query && !isset($requestUri))) {
                $uri .= '?'.$query;
            }
        }
        ...
        foreach (Mage::app()->getStores() as $store) {
            if ($store->getIsActive()) {
                ...
                if (!$this->getConfig()->getCacheEnabled($store->getId())) {
                    $storeInfo[] = 'Full Page Cache disabled for "'.parent::escapeHtml($store->getName()).'" store — '.$store->getBaseUrl().'&nbsp;&nbsp;&nbsp;( ID: '.$store->getId().')
