...
class Mirasvit_Fpc_Helper_Message extends Mage_Core_Helper_Abstract
{
    public function addMessage(&$content, $message = false, $messageType = false) {
        ...
        if ($message && $action == 'catalog/category_view') {
            ...
            && preg_match('/<div\s+id="messages_product_view">(.*?)<\\/div>/ims', $content, $parsedMessage)
            && isset($parsedMessage[0])) { //delete message if cache was flushed (need for some themes)
                $content = preg_replace('/<div\s+id="messages_product_view">(.*?)<\\/div>/ims',
