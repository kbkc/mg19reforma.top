...
class Mirasvit_Fpc_Helper_Debug extends Mage_Core_Helper_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->_debugIgnoredurlparams = Mage::helper('fpc/debug_ignoredurlparams');
    }
    ...
    public function appendDebugInformation(&$content, $isHit = 0, $storage = false)
    {
        ...
        if ($isHit == 2) {
            $info .= '<h2 class="m-fpc-action">Action: ' . $this->getCurrentFullActionCode() . '</h2>';
        } else {
            $info .= '
        ...
        $ignoredUrlParams = $this->_debugIgnoredurlparams->isIgnoredUrlParamApplied(
            $this->_config->getIgnoredUrlParams()
        );

        if ($isUseCache && $isCacheEnabled && $ignoredUrlParams) {
            $info .= '<h2 class="m-fpc-ignored-url-parameters-info">Ignored url parameters applied: '
                . $ignoredUrlParams
                . '<br/>FPC creates the same cache for this page as for <br/>' .  Mage::helper('fpc')->getNormalizedUrl(true)
                . '</h2>';
        }
    ...
    public function getStorageData($storage, $isHit)
    {
        $detailInfo = array(
            ...
            'Action'                                      => $this->getCurrentFullActionCode(),
            ...
            'Update ignored url parameters'               => $this->getTimer('FPC_UPDATE_IGNORED_URL_PARAMS'),
        );

        return $detailInfo;
    }

    protected function getCurrentFullActionCode()
    {
        return (Mage::registry(Mirasvit_Fpc_Model_Config::FPC_FULL_ACTION_CODE))
            ? Mage::registry(Mirasvit_Fpc_Model_Config::FPC_FULL_ACTION_CODE)
            : Mage::helper('fpc')->getFullActionCode();
    }
