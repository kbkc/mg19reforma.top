...
class Mirasvit_Fpc_Model_Config extends Varien_Simplexml_Config
{
    ...
    const USE_SAME_CACHE_LOGGED_IN = 1;
    const USE_SAME_CACHE_ALL = 2;
    ...
    const HTML_NAME_PATTERN_DEFAULT = 1;
    const HTML_NAME_PATTERN_VAR_ONE = 2;

    const HTML_NAME_PATTERN = '/<\[!--\{(.*?)\}--\]>/i';
    const HTML_NAME_PATTERN_ONE = '/<!--\[unique_tag_start_begin(.*?)unique_tag_start_end\]-->/i';

    const REPORT_BLOCK = 'reports/product_viewed';

    const FPC_FULL_ACTION_CODE = 'fpc_current_full_action_code';

    const FIRST_PAGE = 'p=1';

    const MOBILE_DETECT_MOBILE = 1;
    const MOBILE_DETECT_MOBILE_TABLET = 2;

    protected $ignoredValuesForIgnoredParams = array('price',
        'limit',
        'order',
        'p',
        'dir',
    ...
    public function getIgnoredUrlParams()
    {
        ...
        $ignoredUrlParams = $this->_prepareValues($key);
        $ignoredUrlParamsPrepared = array();

        foreach ($ignoredUrlParams as $key => $param) {
            if (!in_array($param, $this->ignoredValuesForIgnoredParams)
                && (strpos($param, 'p=') === false || strpos($param, self::FIRST_PAGE) !== false)) { //can be used p=1
                $ignoredUrlParamsPrepared[] = $param;
            }
        }

        return $ignoredUrlParamsPrepared;
    }
    ...
    public function isCrawlMobileUrls()
    {
        return Mage::getStoreConfig('fpc/cache_rules/is_crawl_mobile_urls');
    }
    ...
    /**
     * If enabled delete ignored url params for layered navigation.
     *
     * @return int
     */
    public function isDeleteIgnoredUrlParams()
    {
        return Mage::getStoreConfig('fpc/extended_settings/delete_ignored_url_params');
    }

    /**
     * @return int
     */
    public function getHtmlNamePattern()
    {
        if ($this->getHtmlNamePatternConfig() == self::HTML_NAME_PATTERN_DEFAULT) {
            return self::HTML_NAME_PATTERN;
        }

        return self::HTML_NAME_PATTERN_ONE;
    }

    /**
     * @return int
     */
    public function addRelatedProductsTags()
    {
        return Mage::getStoreConfig('fpc/extended_settings/add_related_products_tags');
    }

    /**
     * @return int
     */
    public function getHtmlNamePatternConfig()
    {
        return Mage::getStoreConfig('fpc/extended_settings/html_name_pattern');
    }
