...
abstract class Mirasvit_Fpc_Model_Container_Abstract
{
    ...
    //dependences values
    protected static $_pageUrl = null;
    ...
    {
        if ($this->getConfig()->getHtmlNamePatternConfig() == Mirasvit_Fpc_Model_Config::HTML_NAME_PATTERN_DEFAULT) {
            return '<[!--{'.$this->getDefinitionHash().'}--]>';
        }

        return '<!--[unique_tag_start_begin'.$this->getDefinitionHash().'unique_tag_start_end]-->';
    }
    ...
    protected function _getEndReplacerTag()
    {
        ...
        return '<!--[unique_tag_end_begin'.$this->getDefinitionHash().'unique_tag_end_end]-->';
    }
    ...
    protected function getCustomerGroup()
    {
        if (self::$_customerGroup === null) {
            ...
            self::$_customerGroup = Mage::helper('fpc/useSameCache')->getFpcCustomerGroup(self::$_customerGroup);
            Mage::helper('fpc/debug')->stopTimer('FPC_DEPENDENCES_' . $this->_prepareDependenceName(__FUNCTION__));
        }
