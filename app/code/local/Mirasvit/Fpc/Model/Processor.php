...
class Mirasvit_Fpc_Model_Processor
{
    ...
    protected static $_isPageFound = null;
    ...
    public function __construct()
    {
        ...
        $this->_frontupdateHelper = Mage::helper('fpc/processor_frontupdate');
    ...
    public function serveResponse()
    {
        ...
        if ($this->_storage->load()) {
            ...
            if ($this->_storage->getCurrentProduct()) {
                ...
                //Mirasvit Action compatibility
                if (Mage::helper('fpc/compatibility_action')->isActionApplied()) {
                    $this->_unregister();
            ...
            $containers = array();
            preg_match_all(
                $this->_config->getHtmlNamePattern(),
            ...
            if ($this->_frontupdateHelper->isFrontUpdateEnabled()
                && ($categoryProductIds = $this->_storage->getCategoryProductIds())) {
                $categoryHash = $this->_frontupdateHelper->getCategoryListingHash($categoryProductIds);
                $categoryStorageHash = $this->_storage->getCategoryListingHash();
                if ($categoryProductIds && $categoryHash != $categoryStorageHash) {
                    $this->_unregister();
            ...
            $this->_responseHelper->deleteWrongData($content);
            $this->_responseHelper->updateZopimInfo($content);
            $this->_addMessageText($content);
            if ($this->_config->isDeleteIgnoredUrlParams()) {
                $this->_responseHelper->updateIgnoredUrlParams($content);
            }
    ...
    protected function _applyTemplateBlocks()
    {
        ...
        foreach ($keys as $key => $value) {
            ...
            if (isset($value[0])
                ...
                && strpos($containers[$key][$value[0]]['container'], $blockNeededValue) !== false
            ) {
    ...
    protected function addHomePageTags()
    {
        ...
        if (Mage::registry('m_fpc_register_product_tags_added')
            ...
            && $this->_config->getCacheTagslevelLevel() != Mirasvit_Fpc_Model_Config::CACHE_TAGS_LEVEL_EMPTY) {
            $variable = Mage::getModel('core/variable')->loadByCode($code);
    ...
    public function cacheResponse()
    {
        ...
        //Pimgento_Product compatibility
        if (Mage::registry('m__fpc_category_hash') && Mage::registry('m__fpc_category_product_ids')) {
            $this->_storage->setCategoryListingHash(Mage::registry('m__fpc_category_hash'));
            $this->_storage->setCategoryProductIds(Mage::registry('m__fpc_category_product_ids'));
        }
    ...
    public function markContainer($observer)
    {
        ...
        if ($blockName == 'product_list'
            && $this->_isCategoryPage()
            && !Mage::registry('m__fpc_category_product_ids')
            && $this->_frontupdateHelper->isFrontUpdateEnabled()) {
            $productCollection = $observer->getData('block')->getLoadedProductCollection();
            $productIds = $productCollection->getAllIds();
            Mage::register('m__fpc_category_product_ids', $productIds);
            Mage::register('m__fpc_category_hash', $this->_frontupdateHelper->getCategoryListingHash($productIds));
        }
    ...
    public function canProcessRequest($request = null)
    {
        ...
        if ($result) {
            $messageTotal = Mage::getSingleton('core/session')->getMessages()->count()
                ...
                // uncomment for exceptional message issues
