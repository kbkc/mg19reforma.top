...
class Mirasvit_Fpc_Block_Adminhtml_OptimalConfiguration extends Mage_Adminhtml_Block_Template
{
    ...
    public function getMaxCacheSize()
    {
        ...
        if (($freeHddSpace === 0 || $freeHddSpace > 0)
            ...
            if ($maxCacheSize <= 0) {
                return 'You have only '.ceil($freeHddSpace).'Mb free space';
            }
    ...
    protected function getAdditionalPerformanceInfo()
    {
        ...
        if ($this->getProductsCount() < 2000 && !$this->isRedisInstalled()) {
            $info = '. To improve this valuse try switch store cache to  <a target="_blank" 
              href="https://docs.mirasvit.com/doc/extension_fpc/current/cachesupport">database</a> or install 
              <a target="_blank" href="https://docs.mirasvit.com/doc/extension_fpc/current/cachesupport">REDIS</a>.';
        } elseif ($this->getProductsCount() >= 2000 && !$this->isRedisInstalled()) {
            $info = '. To improve this value try install <a target="_blank" href="https://docs.mirasvit.com/doc/extension_fpc/current/cachesupport">REDIS</a>.';
        }
