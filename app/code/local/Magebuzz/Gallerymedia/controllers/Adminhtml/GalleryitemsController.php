...
class Magebuzz_Gallerymedia_Adminhtml_GalleryitemsController extends Mage_Adminhtml_Controller_action
{
  ...
  public function saveAction()
  {
    if ($data = $this->getRequest()->getPost()) {
      ...
      if (isset($data['in_albums']) && is_array($data['in_albums'])) {
        $albumIds = array();
        parse_str(implode(',',$data['in_albums']), $albumIds);
        $albumIds = array_keys($albumIds);
      } else {
        $albumIds = array(0);
      }
     // echo var_dump($albumIds);
     // die();
      try {
        ...
        if (isset($data['in_albums'])) {
          Mage::helper('gallerymedia')->assignMediaToAlbum($model, $data['in_albums']);
        }
