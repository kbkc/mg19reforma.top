...
class Zitec_Dpd_Model_Mysql4_Carrier_Tablerate extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getRate(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if (empty($rates)) {
            $rates = Mage::getModel('zitec_dpd/config_source_service')->getDefaultShippingRates();
        }
    ...
    protected function _validateMethod($method)
    {
        if (!is_array($this->_validMethods)) {
            $this->_validMethods = array_keys(Mage::getModel('zitec_dpd/config_source_service')->getAvailableServices());
        }
